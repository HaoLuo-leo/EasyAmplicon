...
## 1. Start files (起始文件)
    ...
    # 1. Analysis pipeline: pipeline.sh (分析流程pipeline.sh)
    # 2. Sample metadata: metadata.txt, saved in the result directory (样本元信息metadata.txt，保存于result目录)
    # 3. Sequencing data: FASTQ files are saved in the seq directory, usually ending with `.fq.gz`, one file per sample.
    # (测序数据fastq文件保存于seq目录，通常以`.fq.gz`结尾，每个样品一对文件)
    ...
    # (进入对应工作目录，创建临时文件存储目录，分析结束可删除)
    cd Pacbio
    mkdir -p seq result temp 

### 1.1. Metadata/Experimental Design (元数据/实验设计)

    # Prepare the sample metadata file: result/metadata.txt (准备样本元数据result/metadata.txt)
...
### 1.2. Sequencing data (测序数据)
    ...
    # Use seqkit to get statistics of the sequencing data. (统计测序数据，依赖seqkit程序)
    seqkit stat seq/feces1.fastq
    # Batch process all sequencing files and summarize the statistics in a table. (批量统计测序数据并汇总表)
...
### 1.3. Pipeline & Database (流程和数据库)
    ...
    # Available 16S/18S/ITS databases for usearch: (usearch可用的16S/18S/ITS数据库：)
    ...
    # Save SILVA_modified.fasta to ${db}/usearch/ (将SILVA数据库SILVA_modified.fasta保存在${db}/usearch/)

    # Save silva_nr99_v138.1_train_DADA2.fa.gz to ${db}/DADA2/ (将SILVA数据库silva_nr99_v138.1_train_DADA2.fa.gz保存在${db}/DADA2/)
...
### 2.1 Reads rename (文件改名)
    ...
    # # Example of renaming a single sequence file (单个序列改名示例)
    # i=feces1
    ...
    # # For large datasets, refer to "FAQ 2" for the vsearch method. (vsearch大数据方法参考“常见问题2”)

### 2.2 Integrate renamed reads (改名后序列整合)

    # Merge all samples into a single file (合并所有样品至同一文件)
...
## 3. Cut primers and quality filter (切除引物与质控)
    ...
    # Note: Switch from Git Bash to Linux Bash (注意将gitbash切换为linux bash)
    cd Pacbio
    ...
    # View the file to understand the FASTA format (查看文件了解fa文件格式)
...
### 4.1 Dereplicate sequences (序列去冗余)
    ...
    # View the uniques.fa file (查看uniques.fa文件)
...
### 4.2 Cluster OTUs / Denoise ASVs (聚类OTU/去噪ASV)
    ...
    # Results took 3s, produced 264 OTUs, and removed 4 chimeras. (结果耗时3s, 产生264 OTUs, 去除4chimeras)
    ...
    # Rename sequences: Change Zotu to ASV for easy identification. (修改序列名：Zotu改为ASV方便识别)
...
### 4.3 Reference-based chimera detection (基于参考去嵌合)
    ...
    # Method 1. Chimera removal with vsearch + SILVA (方法1. vsearch+silva去嵌合)
    ...
    # Method 2. Do not remove chimeras (方法2. 不去嵌合)
...
### 5.1 Generate a Feature table (生成特征表)
    ...
    # Method 2. vsearch generates a feature table (方法2. vsearch生成特征表)
    ...
    # Use csvtk to count the rows and columns of the table. (csvtk统计表行列)
...
### 5.2 Taxonomic annotation, and/or removal of plastids and non-Bacteria (物种注释，且/或去除质体和非细菌)
    ...
    # Method 1. Number of rows in the original feature table (方法1. 原始特征表行数)
    ...
    # Number of rows in the feature table after filtering (筛选后特征表行数)
    wc -l result/otutab.txt
    # Filter the corresponding sequences in the feature table (过滤特征表对应序列)
    ...
    # Filter the corresponding sequence annotations in the feature table (过滤特征表对应序列注释)
    awk 'NR==FNR{a[$1]=$0}NR>FNR{print a[$1]}'\
        result/raw/otus.sintax result/otutab.id \
        > result/otus.sintax

    # Method 2. If you think the filtering is unreasonable, you can skip it. (方法2. 觉得筛选不合理可以不筛选)
    # cp result/raw/otu* result/

    # Optional statistical method: Simple summary of the OTU table (可选统计方法：OTU表简单统计 Summary OTUs table)
...
### 6.3. Filter by abundance (筛选高丰度菌)
    ...
    # -h displays the script help (parameter description). (-h显示脚本帮助(参数说明))
    ...
    # The result is the mean of all samples and each group. (结果为全部和各组均值)
    ...
    # Try it: How many OTUs/ASVs are there in each group at different abundances? (试一试：不同丰度下各组有多少OTU/ASV)
...
## 7. Beta diversity (β多样性)
    ...
    # The results have multiple files, so a directory is needed. (结果有多个文件，需要目录)
    mkdir -p result/beta/
    # Build a phylogenetic tree based on OTUs. Takes 6.7s. (基于OTU构建进化树 Make OTU tree, 6.7s)
...
## 9. Reference-based quantitative feature table (有参定量特征表)
    ...
    # Alignment rate 80.8%, 4 cores, 1m, memory usage 731Mb. (比对率80.8%, 4核1m, 内存使用731Mb)
    ...
    # Alignment rate 83.83%, 12 cores, 4m 24s. (比对率83.83%, 12核4m24s)
...
## 10. Workspace cleanup and data submission (空间清理及数据提交)
    ...
    # Calculate md5 values for both ends for data submission (分双端统计md5值，用于数据提交)
    cd seq
...
# (DADA2使用朴素贝叶斯分类算法结合数据库注释ASVs，其可能会出现假阳性)
# Users can choose according to their own needs. (用户可根据自己的需求选择)
```
...
```
     ...
     # 定义输入、输出、元数据和R脚本的路径
       ...
       # --- DADA2_PB.R --help information --- (DADA2_PB.R 的 --help 帮助信息)
       ...
       #   --input_dir INPUT_DIR
       #                         Path to the directory containing input FASTQ files. (输入FASTQ文件所在目录的路径)
       #   --output_dir OUTPUT_DIR
       #                         Path to the directory where results will be saved. (用于保存结果的目录路径)
       #   --metadata_file METADATA_FILE
       #                         Path to the metadata file. Must contain a 'SampleID' column. (元数据文件路径，必须包含'SampleID'列)
       #   --fwd_primer FWD_PRIMER
       #                         Forward primer sequence (e.g., 27F). (正向引物序列，例如27F)
       #   --rev_primer REV_PRIMER
       #                         Reverse primer sequence (e.g., 1492R). (反向引物序列，例如1492R)
       #   --min_len MIN_LEN     Minimum length of reads to keep after filtering. (过滤后保留序列的最小长度)
       #   --max_len MAX_LEN     Maximum length of reads to keep after filtering. (过滤后保留序列的最大长度)
...
# PICRUSt2 Functional Prediction (PICRUSt2功能预测)
    ...
    # Method 2. Import installation environment (recommended) (方法2. 导入安装环境(推荐))
    n=picrust2
    # Copy the installation package, or download my packaged environment (复制安装包，或下载我的环境打包)
    wget -c ftp://download.nmdc.cn/tools/conda/${n}.tar.gz
    # Specify the installation directory and decompress (指定安装目录并解压)
    condapath=~/miniconda3
    mkdir -p ${condapath}/envs/${n}
    tar -xvzf ${n}.tar.gz -C ${condapath}/envs/${n}
    # Activate the environment and initialize (激活环境并初始化)
    conda activate picrust2
    conda unpack
    # Load the environment (加载环境)
    conda activate picrust2
    ...
    # Add EC/KO/Pathway annotations (添加EC/KO/Pathway注释)
    cd out
    ...
    # Merge KEGG by hierarchy (KEGG按层级合并)
    ...
    python3 ${db}/script/summarizeAbundance.py \
      -i KEGG.KO.txt \
        ...
        -c 2,3,4 -s ',+,+,' -n raw \
        -o KEGG
    # Count the number of features at each level (统计各层级特征数量)
    wc -l KEGG*

# Evolution Tree (Evolution进化树)

    # Switch to gitbash (切换gitbash)
    cd ${wd}/PacBio
    mkdir -p result/tree
...
## Filter high-abundance/specified features (筛选高丰度/指定的特征)
    ...
    # Count the number of ASVs in the feature table, e.g., a total of 1234. (统计特征表中ASV数量，如总计1234个)
    tail -n+2 ../otutab_rare.txt | wc -l
    # Filter high-abundance OTUs by a relative abundance of 0.2%. (按相对丰度0.2%筛选高丰度OTU)
    usearch -otutab_trim ../otutab_rare.txt \
        -min_otu_freq 0.002 \
        -output otutab.txt
    # Count the number of features in the filtered OTU table, a total of ~105. (统计筛选OTU表特征数量，总计~105个)
    tail -n+2 otutab.txt | wc -l

    # Method 2. Filter by quantity (方法2. 按数量筛选)
    # # Sort by abundance, from largest to smallest by default. (按丰度排序，默认由大到小)
    ...
    # Modify the feature ID column name (修改特征ID列名)
    sed -i '1 s/\#OTU ID/OTUID/' otutab.txt
    # Extract IDs for sequence extraction (提取ID用于提取序列)
    ...
    ## Filter OTU for species annotation (筛选OTU对物种注释)
    ...
    # Merge species annotation and abundance into an annotation file. (合并物种注释和丰度为注释文件)
...
## Build a phylogenetic tree (构建进化树)
    ...
    # Use Muscle for sequence alignment, takes 3s. (Muscle软件进行序列对齐，3s)
    ...
    ### Method 2. FastTree for fast tree building (Linux) (方法2. FastTree快速建树(Linux))
